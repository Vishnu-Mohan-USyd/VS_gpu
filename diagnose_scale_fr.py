#!/usr/bin/env python
"""Quick diagnostic: test F>R growth at different W_e_e scales.

Runs 200 presentations at each scale and measures F>R trajectory.
Goal: find the optimal scale for F>R learning.
"""
import sys
import time
import math
...
from network_jax import (
    ...
    reset_state_jax, evaluate_tuning_jax, calibrate_ee_drive_jax,
...
def test_scale(scale, state_base, static_base, pref, n_pres=200):
    """Train at given scale for n_pres presentations and return F>R trajectory."""
    M = int(static_base.M)
...
def main():
    print("=" * 70)
    print("Scale vs F>R Diagnostic (200 presentations)")
    print("=" * 70)
    ...
    test_scales = [300, 400, 500, 600, 700, 750, 800, 840]

    print(f"\nTesting {len(test_scales)} scales, 200 presentations each...")
    print("-" * 70)

    for scale in test_scales:
        results, elapsed, cal_mean, w_max = test_scale(
            scale, state, static, pref, n_pres=200)
        trajectory = ' → '.join(f'{r:.4f}' for _, r in results)
        rate_per_100 = (results[-1][1] - results[0][1]) / 2.0
        projected_800 = results[0][1] + rate_per_100 * 8.0
        print(f"  scale={scale:4d} | cal_mean={cal_mean:.3f} | w_max={w_max:.2f} | "
              f"F>R: {trajectory} | rate/100={rate_per_100:+.4f} | "
              f"proj800={projected_800:.3f} | {elapsed:.1f}s")
